...
def _knn(arr1: np.ndarray, arr2: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
    ...
    # print(distances)
    # distances = distances[indices].reshape(arr1.shape[0], k) # 수정 by EEM 230611
    # distances = np.take_along_axis(distances, indices, axis=0) # 수정 by EEM 230611
